...
class ndPlugin : public ndThread, public ndSerializer {
 public:
  ...
  ndPlugin(Type type, const string &tag,
           const Params &params);
  virtual ~ndPlugin();
...
class ndPluginSinkPayload {
 public:
  ...
  inline static ndPluginSinkPayload *Create(
      const json &j, const ndPlugin::Channels &channels,
      uint8_t flags = ndPlugin::DF_NONE) {
    string output;
...
class ndPluginProcessor : public ndPlugin {
 public:
  ...
  virtual void DispatchProcessorEvent(
      Event event, ndInterfaces *interfaces) {}
  virtual void DispatchProcessorEvent(
      Event event, const string &iface,
      ndPacketStats *stats) {}
  virtual void DispatchProcessorEvent(
      Event event, ndPacketStats *stats) {}
  virtual void DispatchProcessorEvent(
      Event event, ndInstanceStatus *status) {}
  virtual void DispatchProcessorEvent(Event event) {}
 ...
 protected:
  ...
  inline void DispatchSinkPayload(
    ...
    DispatchSinkPayload(target, channels, payload.size(),
                        &payload[0], flags);
  }
