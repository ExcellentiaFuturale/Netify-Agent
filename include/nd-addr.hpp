...
public:
    ...
    static void
    Create(ndRadixNetworkEntry<N> &entry, const string &addr) {
        Create(ndAddr(addr));
    }
...
public:
    ...
    inline void Classify(ndAddr::Type &type, const string &addr) {
        Classify(type, ndAddr(addr));
    }

    size_t GetInterfaceAddresses(const string &iface,
      set<string> &result, sa_family_t family = AF_UNSPEC);
...
public:
    ...
    ndInterface(const ndInterface &iface)
      : ifname(iface.ifname), ifname_peer(iface.ifname_peer),
        ...
        case ndCT_NFQ: config_nfq = iface.config_nfq; break;
#endif
        default: break;
        }
    }
...
#endif
#if defined(_ND_USE_NFQUEUE)
        case ndCT_NFQ: return (config_nfq == i.config_nfq);
#endif
        }
        return true;
    }
