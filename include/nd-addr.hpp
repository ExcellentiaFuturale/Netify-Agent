...
class ndAddr {
 public:
  ...
  static bool Create(ndAddr &a, const uint8_t *hw_addr,
                     size_t length);
  ...
  inline bool HasValidPrefix(void) const {
    return (prefix > 0 && ((addr.ss.ss_family == AF_INET &&
                            prefix <= _ND_ADDR_BITSv4) ||
  ...
  struct ndAddrEqual {
    bool operator()(const ndAddr &a1,
                    const ndAddr &a2) const {
...
class ndRadixNetworkEntry {
 public:
  ...
  static bool Create(ndRadixNetworkEntry<N> &entry,
                     const ndAddr &addr) {
    if (!addr.IsValid()) {
    ...
    switch (N) {
      ...
      case _ND_ADDR_BITSv6:  // AF_INET6
        entry.addr |=
            ntohl(addr.addr.in6.sin6_addr.s6_addr32[0]);
        entry.addr <<= _ND_ADDR_BITSv4;
        entry.addr |=
            ntohl(addr.addr.in6.sin6_addr.s6_addr32[1]);
        entry.addr <<= _ND_ADDR_BITSv4;
        entry.addr |=
            ntohl(addr.addr.in6.sin6_addr.s6_addr32[2]);
        entry.addr <<= _ND_ADDR_BITSv4;
        entry.addr |=
            ntohl(addr.addr.in6.sin6_addr.s6_addr32[3]);
        entry.addr &= mask;
        return true;
    }
  ...
  bool operator[](int n) const { return addr[(N - 1) - n]; }
...
ndRadixNetworkEntry<N> radix_substr(
    const ndRadixNetworkEntry<N> &entry, int offset,
    int length) {
  bitset<N> mask;
...
typedef radix_tree<ndRadixNetworkEntry<_ND_ADDR_BITSv4>,
                   ndAddr::Type>
    nd_rn4_atype;
typedef radix_tree<ndRadixNetworkEntry<_ND_ADDR_BITSv6>,
                   ndAddr::Type>
    nd_rn6_atype;
...
typedef unordered_set<ndAddr, ndAddr::ndAddrHash,
                      ndAddr::ndAddrEqual>
    ndInterfaceAddrs;
...
class ndInterfaceAddr : public ndSerializer {
 public:
  ...
  inline bool FindFirstOf(sa_family_t family,
                          ndAddr &addr) const {
    lock_guard<mutex> ul(lock);
    for (auto &it : addrs) {
  ...
  inline bool FindAllOf(const vector<sa_family_t> &families,
...
typedef unordered_map<ndAddr, ndInterfaceAddr,
                      ndAddr::ndAddrHash,
                      ndAddr::ndAddrEqual>
    ndInterfaceEndpoints;
...
class ndInterface : public ndSerializer {
 public:
  ndInterface(const string &ifname, uint32_t capture_type,
  ...
  ndInterface(const ndInterface &iface)
      : ifname(iface.ifname),
        ifname_peer(iface.ifname_peer),
        capture_type(iface.capture_type),
        role(iface.role) {
    endpoint_snapshot = false;
  ...
  template <class T>
  void EncodeAddrs(T &output, const vector<string> &keys,
  ...
  inline bool PushEndpoint(const ndAddr &mac,
                           const ndAddr &ip) {
    lock_guard<mutex> ul(lock);
    auto result =
        endpoints[endpoint_snapshot.load()].emplace(
  ...
  template <class T>
  inline void EncodeEndpoints(bool snapshot,
                              T &output) const {
    lock_guard<mutex> ul(lock);
  ...
  inline void GetEndpoints(
    ...
    for (auto &i : endpoints[snapshot]) {
      vector<string> ip_addrs;
      if (!i.second.FindAllOf({AF_INET, AF_INET6},
                              ip_addrs))
        continue;
