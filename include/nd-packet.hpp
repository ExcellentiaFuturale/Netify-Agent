...
class ndPacket {
 public:
  ...
  ndPacket(const status_flags &status,
...
class ndPacketStats : public ndSerializer {
 public:
  ...
  inline ndPacketStats &operator+=(
      const ndPacketStats &rhs) {
    pkt.raw += rhs.pkt.raw;
    pkt.eth += rhs.pkt.eth;
  ...
  void Encode(T &output) const {
    ...
    serialize(output, {"capture_dropped"},
              pkt.capture_dropped);
