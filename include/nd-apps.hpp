...
typedef unordered_map<nd_app_id_t, ndApplication *>
    nd_app_id_map;
typedef unordered_map<string, nd_app_id_t> nd_domains_t;
typedef unordered_map<string, pair<regex *, string>>
    nd_domain_rx_xforms_t;
...
class ndSoftDissector {
 public:
  ...
  ndSoftDissector(signed aid, signed pid,
                  const string &expr)
...
class ndApplications : public ndSerializer {
 public:
  ...
  void Encode(T &output) const {
    ...
    serialize(output, {"signatures", "soft_dissectors"},
              stats.sc);
 ...
 protected:
  ...
  bool AddDomainTransform(const string &search,
