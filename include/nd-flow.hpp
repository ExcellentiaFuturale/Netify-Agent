...
#define ND_FLOW_TLS_CNLEN \
    256  // TLS SNI hostname/common-name length. Reference: RFC 4366
...
public:
    ...
    void Encode(T &output, const ndFlowStats &stats,
      uint8_t encode_includes = ENCODE_ALL) const {
        ...
        switch (other_type) {
        case OTHER_LOCAL: _other_type = "local"; break;
        ...
        case OTHER_BROADCAST:
            ...
            break;
        case OTHER_REMOTE: _other_type = "remote"; break;
        case OTHER_UNSUPPORTED:
            _other_type = "unsupported";
            break;
        case OTHER_ERROR: _other_type = "error"; break;
        }
        ...
        if (encode_includes & ENCODE_METADATA) {
              ...
              detected_protocol == ND_PROTO_QUIC)
            {
                ...
                if (HasTLSServerCN())
                    serialize(output,
                      { "ssl", "server_cn" }, ssl.server_cn);

                if (HasTLSIssuerDN())
                    serialize(output,
                      { "ssl", "issuer_dn" }, ssl.issuer_dn);

                if (HasTLSSubjectDN())
                    serialize(output,
                      { "ssl", "subject_dn" }, ssl.subject_dn);

                if (HasTLSClientJA3())
                    serialize(output,
                      { "ssl", "client_ja3" }, ssl.client_ja3);

                if (HasTLSServerJA3())
                    serialize(output,
                      { "ssl", "server_ja3" }, ssl.server_ja3);

                if (ssl.cert_fingerprint_found) {
                    nd_sha1_to_string(
                      (const uint8_t *)ssl.cert_fingerprint, digest);
                    serialize(output,
                ...
                if (tls_alpn_server_set.load())
                    serialize(output,
                      { "ssl", "alpn_server" }, tls_alpn_server);
            }

            if (HasBTInfoHash()) {
                nd_sha1_to_string(
                  (const uint8_t *)bt.info_hash, digest);
