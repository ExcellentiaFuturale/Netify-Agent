...
void ndFlowStats::UpdateRate(bool lower, uint64_t timestamp,
  ...
  rate = (divisor > 0) ? ((float)total / (float)divisor)
                       : 0.0f;
}
...
ndFlow::ndFlow(nd_iface_ptr &iface)
    : iface(iface),
      ...
      category{ND_CAT_UNKNOWN, ND_CAT_UNKNOWN,
               ND_CAT_UNKNOWN},
      ndpi_flow(NULL),
      http{{0}},
      privacy_mask(0),
...
ndFlow::ndFlow(const ndFlow &flow)
    : iface(flow.iface),
      ...
      lower_map(LOWER_UNKNOWN),
      other_type(OTHER_UNKNOWN),
      lower_mac(flow.lower_mac),
      upper_mac(flow.upper_mac),
      lower_addr(flow.lower_addr),
      upper_addr(flow.upper_addr),
...
void ndFlow::Hash(const string &device, bool hash_mdata,
  ...
  sha1_init(&ctx);
  sha1_write(&ctx, (const char *)device.c_str(),
             device.size());

  sha1_write(&ctx, (const char *)&ip_version,
             sizeof(ip_version));
  ...
  switch (ip_version) {
    case 4:
      ...
      if (lower_addr.addr.in.sin_addr.s_addr == 0 &&
          upper_addr.addr.in.sin_addr.s_addr ==
              0xffffffff) {
