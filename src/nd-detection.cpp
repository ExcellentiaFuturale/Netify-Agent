...
ndDetectionThread::ndDetectionThread(
...
#endif
    ndDNSHintCache *dhc, ndFlowHashCache *fhc,
  ...
  if ((rc = pthread_cond_init(&pkt_queue_cond,
                              &cond_attr)) != 0)
...
void *ndDetectionThread::Entry(void) {
  ...
  do {
    if ((rc = pthread_mutex_lock(&pkt_queue_cond_mutex)) !=
        0)
      throw ndDetectionThreadException(strerror(rc));
...
void ndDetectionThread::ProcessPacket(
  ...
  if (ndEF->detected_protocol == ND_PROTO_UNKNOWN &&
    ...
    ndEF->detected_protocol =
        nd_ndpi_proto_find(ndpi_rc.master_protocol, ndEF);
  }
...
bool ndDetectionThread::ProcessALPN(
  ...
  bool flow_update = false;
  const char *detected_alpn =
      (client) ? ndEFNFP.tls_quic.advertised_alpns
...
void ndDetectionThread::ProcessFlow(
    ndDetectionQueueEntry *entry) {
  ndi.addr_types.Classify(ndEF->lower_type,
                          ndEF->lower_addr);
  ...
  if (dhc != NULL &&
    ...
    if (ndEF->flags.dhc_hit.load()) {
      ...
      if (ndEFNF->host_server_name[0] == '\0' ||
          nd_is_ipaddr(
              (const char *)ndEFNF->host_server_name)) {
        snprintf((char *)ndEFNF->host_server_name,
                 sizeof(ndEFNF->host_server_name) - 1, "%s",
                 hostname.c_str());
      }
    }
  }
  ...
  switch (ndEF->detected_protocol) {
    case ND_PROTO_APPLE_PUSH:
      SetDetectedApplication(
          entry, ndi.apps.Lookup("netify.apple-push"));
      break;

    case ND_PROTO_AVAST:
      SetDetectedApplication(
          entry, ndi.apps.Lookup("netify.avast"));
      break;

    case ND_PROTO_LINE_CALL:
      SetDetectedApplication(
          entry, ndi.apps.Lookup("netify.line"));
      break;

    case ND_PROTO_NEST_LOG_SINK:
      SetDetectedApplication(
          entry, ndi.apps.Lookup("netify.nest"));
      break;

    case ND_PROTO_SPOTIFY:
      SetDetectedApplication(
          entry, ndi.apps.Lookup("netify.spotify"));
      break;

    case ND_PROTO_STEAM:
      SetDetectedApplication(
          entry, ndi.apps.Lookup("netify.steam"));
      break;

    case ND_PROTO_SYNCTHING:
      SetDetectedApplication(
          entry, ndi.apps.Lookup("netify.syncthing"));
      break;

    case ND_PROTO_TEAMVIEWER:
      SetDetectedApplication(
          entry, ndi.apps.Lookup("netify.teamviewer"));
      break;

    case ND_PROTO_TIVOCONNECT:
      SetDetectedApplication(
          entry, ndi.apps.Lookup("netify.tivo"));
      break;

    case ND_PROTO_TPLINK_SHP:
      SetDetectedApplication(
          entry, ndi.apps.Lookup("netify.tp-link"));
      break;

    case ND_PROTO_TUYA_LP:
      SetDetectedApplication(
          entry, ndi.apps.Lookup("netify.tuya-smart"));
      break;

    case ND_PROTO_UBNTAC2:
      SetDetectedApplication(
          entry, ndi.apps.Lookup("netify.ubiquiti"));
      break;

    case ND_PROTO_ZOOM:
      SetDetectedApplication(
          entry, ndi.apps.Lookup("netify.zoom"));
      break;
  ...
  if (ndEF->detected_application == ND_APP_UNKNOWN) {
    if (ndEF->host_server_name[0] != '\0')
      SetDetectedApplication(
          entry, ndi.apps.Find(ndEF->host_server_name));
  }
  ...
  if (ndEF->detected_application == ND_APP_UNKNOWN) {
    if (ndEF->dns_host_name[0] != '\0')
      SetDetectedApplication(
          entry, ndi.apps.Find(ndEF->dns_host_name));
  }
  ...
  if (ndEF->GetMasterProtocol() != ND_PROTO_DNS &&
    ...
    if (ndEF->lower_type == ndAddr::atOTHER) {
      SetDetectedApplication(
          entry, ndi.apps.Find(ndEF->lower_addr));

      if (ndEF->detected_application == ND_APP_UNKNOWN)
        SetDetectedApplication(
            entry, ndi.apps.Find(ndEF->upper_addr));
    } else {
      SetDetectedApplication(
          entry, ndi.apps.Find(ndEF->upper_addr));
  ...
  for (int t = ndFlow::TYPE_LOWER; t < ndFlow::TYPE_MAX;
       t++) {
    const ndAddr *mac, *ip;
    ...
    if (type != ndAddr::atOTHER || mac == nullptr ||
        ip == nullptr)
      continue;
  ...
  if (ndEFNF->host_server_name[0] != '\0') {
    ndEF->category.domain =
        ndi.domains.Lookup(ndEFNF->host_server_name);
  ...
  for (vector<uint8_t *>::const_iterator i =
    ...
    if (!memcmp((*i), ndEF->lower_mac.addr.ll.sll_addr,
                ETH_ALEN))
      ndEF->privacy_mask |= ndFlow::PRIVATE_LOWER;
    if (!memcmp((*i), ndEF->upper_mac.addr.ll.sll_addr,
                ETH_ALEN))
      ndEF->privacy_mask |= ndFlow::PRIVATE_UPPER;
#elif defined(__FreeBSD__)
    if (!memcmp((*i), LLADDR(&ndEF->lower_mac.addr.dl),
                ETH_ALEN))
      ndEF->privacy_mask |= ndFlow::PRIVATE_LOWER;
    if (!memcmp((*i), LLADDR(&ndEF->upper_mac.addr.dl),
                ETH_ALEN))
      ndEF->privacy_mask |= ndFlow::PRIVATE_UPPER;
#endif
  }
  ...
  for (vector<struct sockaddr *>::const_iterator i =
    ...
    switch ((*i)->sa_family) {
      case AF_INET:
        sa_in =
            reinterpret_cast<struct sockaddr_in *>((*i));
      ...
      case AF_INET6:
        sa_in6 =
            reinterpret_cast<struct sockaddr_in6 *>((*i));
...
void ndDetectionThread::SetDetectedApplication(
  ...
  ndEF->category.application =
      ndi.categories.Lookup(ndCAT_TYPE_APP, app_id);
}

void ndDetectionThread::FlowUpdate(
    ndDetectionQueueEntry *entry) {
...
void ndDetectionThread::SetGuessedProtocol(
    ndDetectionQueueEntry *entry) {
  uint8_t guessed = 0;
  ndpi_protocol ndpi_rc =
      ndpi_detection_giveup(ndpi, ndEFNF, 1, &guessed);
