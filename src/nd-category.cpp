...
bool ndCategories::Load(void) {
  ...
  last_update =
      (time_t)(jdata["last_update"].get<unsigned>());
  ...
  for (auto &ci : categories) {
    ...
    if (!key.empty()) {
      ci.second.tag = jdata[key + "_tag_index"]
                          .get<ndCategory::index_tag>();
      ci.second.index = jdata[key + "_index"]
                            .get<ndCategory::index_cat>();
    }
  }
...
bool ndDomains::Load(const string &path_domains) {
  ...
  for (auto &it : files) {
    ...
    if (p1 == string::npos) {
      nd_dprintf(
          "Rejecting domain file (wrong format; missing "
          "hyphen): %s\n",
          it.c_str());
      continue;
    }
    ...
    if (tag == index_tag.end()) {
      nd_dprintf(
          "Rejecting domain file (invalid category tag): "
          "%s\n",
          it.c_str());
      continue;
    }
    ...
    if (!ifs.is_open()) {
      nd_printf(
          "Error opening custom domain category file: %s\n",
          it.c_str());
      continue;
    }
