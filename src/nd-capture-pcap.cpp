...
void *ndCapturePcap::Entry(void) {
  ...
  while (!ShouldTerminate()) {
    if (pcap != nullptr) {
      ...
      tv.tv_sec = 1;
      tv.tv_usec = 0;
      rc = select(sd_max + 1, &fds_read, nullptr, nullptr,
                  &tv);
      ...
      while (ShouldTerminate() == false &&
             (rc = pcap_next_ex(pcap, &pkt_header,
                                &pkt_data)) > 0) {
      ...
      if (rc < 0) {
        ...
        if (rc == -1) {
          nd_printf("%s: %s.\n", tag.c_str(),
                    pcap_geterr(pcap));
          if (ndCT_TYPE(iface->capture_type) ==
              ndCT_PCAP_OFFLINE)
            Terminate();
          else {
            pcap_close(pcap);
...
pcap_t *ndCapturePcap::OpenCapture(void) {
  ...
  if (ndCT_TYPE(iface->capture_type) == ndCT_PCAP_OFFLINE) {
    ...
    pcap_new = pcap_open_live(
        tag.c_str(), ndGC.max_capture_length, 1,
        ndGC.capture_read_timeout, pcap_errbuf);
  ...
  if (pcap_new == nullptr)
    ...
    if (ndCT_TYPE(iface->capture_type) !=
        ndCT_PCAP_OFFLINE) {
      if (pcap_setnonblock(pcap_new, 1, pcap_errbuf) ==
          PCAP_ERROR)
