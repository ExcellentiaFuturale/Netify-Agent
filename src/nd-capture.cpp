...
ndCaptureThread::ndCaptureThread(
    unsigned cs_type, int16_t cpu, nd_iface_ptr &iface,
...
const ndPacket *ndCaptureThread::ProcessPacket(
  ...
  switch (dl_type) {
    case DLT_NULL:
      if (packet->caplen < sizeof(uint32_t)) {
        ...
        nd_dprintf("%s: discard: Truncated packet.",
                   tag.c_str());
#endif
        return packet;
      }
      ...
      switch (ntohl(*((uint32_t *)packet->data))) {
        ...
        default:
          ...
          stats.pkt.discard_bytes += packet->length;
          nd_dprintf(
              "%s: discard: Unsupported BSD loopback "
              "encapsulation type: %lu\n",
              tag.c_str(),
              ntohl(*((uint32_t *)packet->data)));
#endif
          return packet;
      }
    ...
    case DLT_EN10MB:
      ...
      hdr_eth =
          reinterpret_cast<const struct ether_header *>(
              packet->data);
...
#ifdef _ND_LOG_PKT_DISCARD
      nd_dprintf(
          "%s: discard: Unsupported datalink type: 0x%x\n",
...
#ifdef _ND_LOG_PKT_DISCARD
    nd_dprintf(
        "%s: discard: layer-2 length is beyond capture "
        "length.\n",
        tag.c_str());
#endif
  }
  ...
  while (true) {
    if (type == ETHERTYPE_VLAN) {
...
#ifdef _ND_LOG_PKT_DISCARD
        nd_dprintf(
            "%s: discard: layer-2 + VLAN length is beyond "
            "capture length.\n",
            tag.c_str());
#endif
      }
      ...
      l2_len += VLAN_TAG_LEN;
    } else if (type == ETHERTYPE_MPLS_UC ||
               type == ETHERTYPE_MPLS_MC) {
      stats.pkt.mpls++;
      union {
        uint32_t u32;
      ...
      mpls.u32 =
          ntohl(*((uint32_t *)&packet->data[l2_len]));
      type = ETHERTYPE_IP;
      l2_len += 4;
...
#ifdef _ND_LOG_PKT_DISCARD
        nd_dprintf(
            "%s: discard: layer-2 + MPLS length is beyond "
            "capture length.\n",
            tag.c_str());
#endif
      }
...
#ifdef _ND_LOG_PKT_DISCARD
          nd_dprintf(
              "%s: discard: layer-2 + MPLS length is "
              "beyond capture length.\n",
              tag.c_str());
#endif
        }
        mpls.u32 =
...
#ifdef _ND_LOG_PKT_DISCARD
        nd_dprintf(
            "%s: discard: layer-2 + PPP length is beyond "
            "capture length.\n",
            tag.c_str());
#endif
      }

      ppp_proto = (uint16_t)(_ND_PPP_PROTOCOL(packet->data +
                                              l2_len + 6));
...
#ifdef _ND_LOG_PKT_DISCARD
        nd_dprintf(
            "%s: discard: unsupported PPP protocol: "
            "0x%04hx\n",
            tag.c_str(), ppp_proto);
#endif
        return packet;
...
nd_process_ip:
...
#ifdef _ND_LOG_PKT_DISCARD
    nd_dprintf(
        "%s: discard: layer-2 + IP length is beyond "
        "capture length.\n",
        tag.c_str());
#endif
  }
  ...
  if (flow.ip_version == 4) {
    ...
    if ((unsigned)(packet->caplen - l2_len) <
        sizeof(struct ip)) {
...
#ifdef _ND_LOG_PKT_DISCARD
      nd_dprintf(
          "%s: discard: l3_len[%hu] > (packet->caplen[%hu] "
          "- l2_len[%hu])(%hu)\n",
    ...
    flow.ip_protocol =
        hdr_ip6->ip6_ctlun.ip6_un1.ip6_un1_nxt;

    if (ndpi_handle_ipv6_extension_headers(
            l3_len, &l4, &l4_len, &flow.ip_protocol)) {
      stats.pkt.discard++;
      stats.pkt.discard_bytes += packet->length;
#ifdef _ND_LOG_PKT_DISCARD
      nd_dprintf(
          "%s: discard: Error walking IPv6 extensions.\n",
          tag.c_str());
#endif
      return packet;
    }
...
#ifdef _ND_LOG_PKT_DISCARD
    nd_dprintf(
        "%s: discard: invalid IP protocol version: %hhx\n",
...
#ifdef _ND_LOG_PKT_DISCARD
    nd_dprintf(
        "%s: discard: L2 + L3 + L4 length is beyond "
        "capture length.\n",
        tag.c_str());
#endif
    return packet;
  ...
  if (l4_len > 8 && flow.ip_protocol == IPPROTO_UDP) {
    ...
    if (ntohs(hdr_udp->uh_sport) == _ND_GTP_U_PORT ||
      ...
      hdr_gtpv1 = reinterpret_cast<
          const struct nd_gtpv1_header_t *>(
      ...
      if (hdr_gtpv1->flags.version == 1) {
        ...
        // TODO: GTPv2...
        hdr_gtpv2 = reinterpret_cast<
            const struct nd_gtpv2_header_t *>(
            l4 + sizeof(struct udphdr));
        nd_dprintf(
            "%s: unimplemented GTP version (TODO): %u\n",
  ...
  switch (flow.ip_protocol) {
    case IPPROTO_TCP:
      ...
      if (l4_len < 20) {
        ...
        hdr_tcp =
            reinterpret_cast<const struct tcphdr *>(l4);
#ifdef _ND_LOG_PKT_TCP_FLAGS
        nd_dprintf(
            "%s: TCP flags: %c%c%c%c%c%c\n", tag.c_str(),
...
#ifdef _ND_LOG_PKT_DISCARD
          nd_dprintf(
              "%s: discard: unexpected TCP payload "
              "length.\n",
              tag.c_str());
#endif
          return packet;
    ...
    case IPPROTO_UDP:
...
#ifdef _ND_LOG_PKT_DISCARD
        nd_dprintf(
            "%s: discard: unexpected UDP data length.\n",
            tag.c_str());
#endif
        return packet;
      }
      ...
      pkt = reinterpret_cast<const uint8_t *>(
          l4 + sizeof(struct udphdr));
      pkt_len =
          ntohs(hdr_udp->uh_ulen) - sizeof(struct udphdr);
  ...
  if (nf) {
    ...
    if (ndGC.max_flows > 0 &&
      ...
      nd_dprintf(
          "%s: discard: maximum flows exceeded: %u\n",
    ...
    if (!ndi.flow_buckets->InsertUnlocked(flow_digest,
                                          nf)) {
      ndi.flow_buckets->Release(flow_digest);
  ...
  if (dhc != NULL && pkt != NULL &&
    ...
    if (lport == 53 || uport == 53)
      ...
      proto = ND_PROTO_LLMNR;

    if (proto != ND_PROTO_UNKNOWN) {
      bool is_query = ProcessDNSPacket(
          nf, pkt,
          pkt_len - (packet->length - packet->caplen),
          proto);
  ...
  if (nf->flags.detection_complete.load() == false &&
      ...
      nf->stats.detection_packets.load() <=
...
bool ndCaptureThread::ProcessDNSPacket(nd_flow_ptr &flow,
  ...
  if (rc < 0) {
#ifdef _ND_LOG_DHC
    nd_dprintf("%s: dns initparse error: %s, length: %hu\n",
  ...
  nd_dprintf(
      ...
      ns_msg_count(ns_h, ns_s_qd),
      ns_msg_count(ns_h, ns_s_an));
#endif

  for (uint16_t i = 0; i < ns_msg_count(ns_h, ns_s_qd);
       i++) {
  ...
  for (uint16_t i = 0; i < ns_msg_count(ns_h, ns_s_an);
       i++) {
    ...
    if (ns_rr_type(rr) == ns_t_ptr) {
      ...
      if (flow->mdns.domain_name[0] != '\0') {
        nd_set_hostname(flow->mdns.domain_name,
                        flow->mdns.domain_name,
                        ND_FLOW_HOSTNAME, false);
      }
#ifdef _ND_LOG_DHC
      if (flow->HasMDNSDomainName() == false) continue;
      nd_dprintf(
          "%s: parsing mDNS PTR RR: ttl: %d, data len: %d: "
          "%s\n",
    ...
    if (proto != ND_PROTO_DNS || host == NULL) continue;

    // Add responses to DHC...
    ndAddr addr(
        (ns_rr_type(rr) == ns_t_a)
            ? ndAddr(
                  (const struct in_addr *)ns_rr_rdata(rr))
            : ndAddr((const struct in6_addr *)ns_rr_rdata(
                  rr)));
