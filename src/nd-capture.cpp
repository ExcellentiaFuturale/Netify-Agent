...
ndCaptureThread::ndCaptureThread(unsigned cs_type, int16_t cpu,
  ...
  : ndThread(iface->ifname, (long)cpu, /* IPC? */ false),
    ndInstanceClient(), dl_type(0), cs_type(cs_type),
    iface(iface), flow(iface), tv_epoch(0), ts_pkt_first(0),
    ts_pkt_last(0), dhc(dhc), threads_dpi(threads_dpi),
...
#ifdef _ND_LOG_PKT_DISCARD
                nd_dprintf(
                  "%s: discard: layer-2 + VLAN length is "
                  "beyond "
                  "capture length.\n",
                  tag.c_str());
#endif
...
#ifdef _ND_LOG_PKT_DISCARD
                nd_dprintf(
                  "%s: discard: layer-2 + PPP length is "
                  "beyond "
                  "capture length.\n",
                  tag.c_str());
#endif
...
#ifdef _ND_LOG_PKT_DISCARD
            nd_dprintf(
              "%s: discard: PPPoE discovery protocol.\n",
              tag.c_str());
#endif
            return packet;
        }
...
nd_process_ip:
...
#ifdef _ND_LOG_PKT_DISCARD
        nd_dprintf(
          "%s: discard: invalid IP protocol version: "
          "%hhx\n",
...
#ifdef _ND_LOG_PKT_DISCARD
            nd_dprintf(
              "%s: discard: TCP header too small.\n", tag.c_str());
#endif
            return packet;
        }
        else {
...
#ifdef _ND_LOG_PKT_DISCARD
            nd_dprintf(
              "%s: discard: UDP header too small.\n", tag.c_str());
#endif
            return packet;
        }
    ...
    if (nf) {
          ...
          hdr_tcp->th_flags != (TH_SYN | TH_ACK))
        {
            ...
            nd_dprintf(
              "%s: discard: new TCP flow without "
              "SYN/ACK.\n",
              tag.c_str());
#endif
            stats.pkt.discard++;
        ...
        if (addr_cmp < 0) nf->origin = ndFlow::ORIGIN_LOWER;
        else nf->origin = ndFlow::ORIGIN_UPPER;
        ...
        if (nf->ip_protocol == IPPROTO_TCP) {
            if ((hdr_tcp->th_flags & TH_SYN)) {
                if (! (hdr_tcp->th_flags & TH_ACK)) {
                    ...
                    if (addr_cmp < 0)
                        nf->origin = ndFlow::ORIGIN_UPPER;
                    else nf->origin = ndFlow::ORIGIN_LOWER;
                }
            }
        }
        ...
        if (ns_rr_type(rr) == ns_t_ptr) {
            ...
            if (flow->mdns.domain_name[0] != '\0') {
                nd_set_hostname(flow->mdns.domain_name,
                  flow->mdns.domain_name, ND_FLOW_HOSTNAME, false);
            }
#ifdef _ND_LOG_DHC
            if (flow->HasMDNSDomainName() == false)
                continue;
