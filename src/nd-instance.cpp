...
uint32_t ndInstance::InitializeConfig(int argc,
                                      char *const argv[]) {
  string last_iface;
...
#define _ND_LO_CA_DETECTION_CORES 9
      ...
      {"thread-detection-base", 1, 0,
       _ND_LO_CA_DETECTION_BASE},
      {"thread-detection-cores", 1, 0,
       _ND_LO_CA_DETECTION_CORES},
...
#define _ND_LO_DISABLE_AUTO_FLOW_EXPIRY 22
  ...
  int rc;
  while (true) {
    if ((rc = getopt_long(argc, argv, flags, options,
                          NULL)) == -1)
      break;
  ...
  while (true) {
    ...
    switch (rc) {
      ...
      case 'c':
        break;
      case 'E':
        if (!AddInterface(optarg, ndIR_WAN,
                          ndCT_PCAP | ndCT_CMDLINE))
      ...
      case 'I':
        if (!AddInterface(optarg, ndIR_LAN,
                          ndCT_PCAP | ndCT_CMDLINE))
          return ndCR_INVALID_INTERFACE;
        last_iface = optarg;
        break;
      ...
      case 'u':
        if (!ndGC.SaveUUID(ndGlobalConfig::UUID_AGENT,
                           optarg)) {
  ...
  if (nd_file_exists(ndGC.path_app_config)) {
    nd_sha1_file(ndGC.path_app_config,
                 ndGC.digest_app_config);
...
bool ndInstance::InitializeTimers(int sig_update,
                                  int sig_update_napi) {
  try {
...
bool ndInstance::Daemonize(void) {
  ...
  if (!nd_dir_exists(ndGC.path_state_volatile)) {
    if (mkdir(ndGC.path_state_volatile.c_str(), 0755) !=
        0) {
      nd_printf(
...
bool ndInstance::SetConfigOption(int option,
                                 const string &arg) {
  ...
  if (files.size() && rc == 0) {
    rc = nd_functions_exec("restart_netifyd", string(),
                           output);
    if (rc != 0) {
      fprintf(stderr,
  ...
  for (auto &entry : entries_by_id) {
    if (type & ndDUMP_WITH_CATS &&
      ...
      nd_cat_id_t cat_id = categories.ResolveTag(
          (type & ndDUMP_TYPE_PROTOS) ? ndCAT_TYPE_PROTO
                                      : ndCAT_TYPE_APP,
          entry.first, tag);
...
void ndInstance::CommandLineHelp(bool version_only) {
  ...
  if (version_only) {
    ...
    fprintf(
        ...
        "\nGlobal options:\n"
        "  -d, --debug\n    Enable debug output and remain "
        "in foreground.\n"
        ...
        "  -U, --uuidgen\n    Generate (but don't save) a "
        "new Agent UUID.\n"
        "  -p, --provision\n    Provision Agent (generate "
        "and save Agent UUID).\n"
        "  -c, --config <filename>\n    Specify an "
        "alternate Agent configuration.\n"
        "    Default: %s\n"
        "  -f, --ndpi-config <filename>\n    Specify an "
        "alternate legacy (nDPI) application configuration "
        "file.\n"
        "    Default: %s\n"
        "  --force-reset\n    Reset global sink "
        "configuration options.\n"
        ...
        "Specify an external (WAN) interface, or file, to "
        "capture from.\n"
        ...
        "Interface/device option: associate interface with "
        ...
        "the number of detection (DPI) threads to start.\n"

        "\nSee netifyd(8) and netifyd.conf(5) for further "
        "options.\n",
...
bool ndInstance::AddInterface(const string &ifname,
  ...
  if ((ndCT_TYPE(type) == ndCT_PCAP ||
    ...
    return ndGC.AddInterface(
        iface, role, ndCT_PCAP_OFFLINE | ndCT_CMDLINE,
...
bool ndInstance::CheckAgentUUID(void) {
  ...
  if (uuid.empty() || uuid == ND_AGENT_UUID_NULL) {
    ...
    if (!ndGC.LoadUUID(ndGlobalConfig::UUID_AGENT, uuid)) {
      ...
      fprintf(stdout, "Generated a new Agent UUID: %s\n",
              uuid.c_str());
...
bool ndInstance::SaveAgentStatus(
    const nd_interface_stats &stats) {
  json jstatus;
...
bool ndInstance::DisplayAgentStatus(void) {
  ...
  fprintf(stderr, "%s volatile state path: %s\n", ND_I_INFO,
          ndGC.path_state_volatile.c_str());
  ...
  try {
    ...
    if (strftime(timestamp, sizeof(timestamp), "%c",
                 tm_local) > 0) {
    ...
    unsigned long flows_in_use =
        (unsigned long)jstatus["flows_in_use"]
            .get<unsigned>();
    ...
    fprintf(stderr,
            ...
            (unsigned long)jstatus["flows_purged"]
                .get<unsigned>(),
    ...
    fprintf(stderr,
            ...
            color, icon, ND_C_RESET,
            (unsigned long)jstatus["flows_expiring"]
                .get<unsigned>(),
    ...
    double cpu_user_percent =
        cpu_user_delta * 100.0 / cpu_max_time;
    double cpu_system_percent =
        cpu_system_delta * 100.0 / cpu_max_time;
    double cpu_total =
    ...
    fprintf(stderr,
            ...
            "%s%.1f%%%s\n",
            color, icon, ND_C_RESET, color, cpu_total,
            ND_C_RESET);
    fprintf(
        stderr,
        "%s%s%s CPU time (user / system): %.1fs / %.1fs\n",
        color, icon, ND_C_RESET, cpu_user_delta,
        cpu_system_delta);
    ...
    if (jstatus.find("interfaces") != jstatus.end()) {
      for (auto &i : jstatus["interfaces"].items()) {
        ...
        try {
          ...
          pkts = jstatus["stats"][iface]["raw"]
                     .get<unsigned>();
          dropped =
              jstatus["stats"][iface]["capture_dropped"]
                  .get<unsigned>();
          dropped +=
              jstatus["stats"][iface]["queue_dropped"]
                  .get<unsigned>();
          ...
          if (pkts == 0) {
            ...
            dropped_percent =
                (double)dropped * 100 / (double)pkts;
    ...
    fprintf(
        ...
        ND_C_RED, ND_I_FAIL, ND_C_RESET, ND_C_RED, e.what(),
        ND_C_RESET);
  }
...
int ndInstance::Run(void) {
  ...
  try {
    ...
    if (ndGC_USE_CONNTRACK) {
      thread_conntrack =
          new ndConntrackThread(ndGC.ca_conntrack);
    ...
    for (int16_t i = 0; i < cpus; i++) {
      thread_detection[i] = new ndDetectionThread(
          cpu, string("dpi") + to_string(cpu),
#ifdef _ND_USE_NETLINK
          netlink,
#endif
#ifdef _ND_USE_CONNTRACK
          (!ndGC_USE_CONNTRACK) ? nullptr
                                : thread_conntrack,
#endif
...
void *ndInstance::ndInstance::Entry(void) {
  ...
  if (ndGC_USE_NETLINK) {
    try {
      ...
      nd_printf(
          "%s: Exception while refreshing Netlink: %s\n",
          tag.c_str(), e.what());
      exit_code = EXIT_FAILURE;
  ...
  try {
    ...
    nd_printf("%s: Exception while starting timer(s): %s\n",
              tag.c_str(), e.what());
    exit_code = EXIT_FAILURE;
    goto ndInstance_EntryReturn;
  }
...
void ndInstance::CreateCaptureInterfaces(
    ndInterfaces &ifaces) {
  for (auto &r : ndGC.interfaces) {
    for (auto &i : r.second) {
      auto result = ifaces.insert(make_pair(
          i.first, make_shared<ndInterface>(
                       i.first, i.second.first, r.first)));
    ...
    switch (ndCT_TYPE(it.second->capture_type)) {
      ...
      case ndCT_PCAP_OFFLINE: {
        ndCapturePcap *thread = new ndCapturePcap(
            (interfaces.size() > 1) ? cpu++ : -1, it.second,
            thread_detection, dns_hint_cache,
            (it.second->role == ndIR_LAN) ? 0
                                          : ++private_addr);
      ...
      case ndCT_TPV3: {
        unsigned instances =
            it.second->config_tpv3.fanout_instances;
      ...
      case ndCT_NFQ: {
        unsigned instances =
            it.second->config_nfq.instances;
...
void ndInstance::DestroyCaptureThreads(
  ...
  for (auto &it : threads) {
    for (auto &it_instance : it.second) delete it_instance;
  }
...
size_t ndInstance::ReapCaptureThreads(
...
bool ndInstance::ReloadCaptureThreads(
    nd_capture_threads &threads) {
  ndInterfaces ifaces, ifaces_new, ifaces_common,
      ifaces_delete;
  ...
  set_difference(ifaces.begin(), ifaces.end(),
                 ...
                 ifaces.value_comp());
  set_intersection(
      ifaces.begin(), ifaces.end(), interfaces.begin(),
      interfaces.end(),
      inserter(ifaces_common, ifaces_common.begin()),
      ifaces.value_comp());
  set_difference(
      interfaces.begin(), interfaces.end(), ifaces.begin(),
      ifaces.end(),
  ...
  for (auto &iface : ifaces_common) {
    ...
    if (*i->second == *iface.second) {
      ...
      if (it != ndGC.interface_addrs.end()) {
        for (auto &a : it->second) ifaddrs_a.insert(a);
      }

      it = old_addrs.find(iface.first);
      if (it != old_addrs.end()) {
        for (auto &a : it->second) ifaddrs_b.insert(a);
      }

      if (ifaddrs_a.empty() && ifaddrs_b.empty()) continue;
  ...
  for (auto &iface : ifaces_delete) {
    ...
    }
    {
      ...
      if (it != interfaces.end()) interfaces.erase(it);
    }
  }
...
int ndInstance::WaitForIPC(int timeout) {
  ...
  do {
    ...
    rc = select(fd_ipc[IPC_PE_READ] + 1, &fds_read, NULL,
                NULL, &tv);

    if (rc == -1) {
      throw ndSystemException(__PRETTY_FUNCTION__, "select",
                              errno);
    }
  ...
  } while (!ShouldTerminate() && timeout < 0 &&
           --timeout != 0);
...
void ndInstance::UpdateStatus(void) {
  ...
  if (clock_gettime(CLOCK_MONOTONIC_RAW, &status.ts_now) !=
      0) {
...
void ndInstance::ProcessUpdate(
    nd_capture_threads &threads) {
  UpdateStatus();
#if !defined(_ND_USE_LIBTCMALLOC) && \
    defined(HAVE_MALLOC_TRIM)
...
void ndInstance::ProcessFlows(void) {
  ...
  for (size_t b = 0; b < buckets; b++) {
    ...
    while (i != fm.end()) {
      ...
      if (i->second->ip_protocol == IPPROTO_TCP &&
        ...
        tcp_fin_ack_gt2++;
#endif
      if (i->second.use_count() > 1) status.flows_in_use++;
      ...
      if (i->second->flags.expired.load() == false) {
        ...
        if (((time_t)(i->second->ts_last_seen.load() /
                      1000) +
             ttl) < now)
  ...
  nd_dprintf(
      ...
      status.flows_expiring, status.flows_expired,
      flows_total);
#ifdef _ND_PROCESS_FLOW_DEBUG
  nd_dprintf(
