...
ndConntrackThread::ndConntrackThread(int16_t cpu)
  : ndThread("nd-conntrack", (long)cpu), ctfd(-1),
    cth(NULL), cb_registered(-1) {
    cth = nfct_open(NFNL_SUBSYS_CTNETLINK, NFCT_ALL_CT_GROUPS);
    if (cth == NULL) {
        if (errno == EPROTONOSUPPORT) {
            nd_printf(
              "%s: nfnetlink kernel module not loaded?\n",
              tag.c_str());
        }
        throw ndConntrackSystemException(__PRETTY_FUNCTION__,
          "nfct_open", errno);
...
void ndConntrackThread::DumpConntrackTable(void) {
    ...
    if (nl == NULL) {
        throw ndConntrackSystemException(__PRETTY_FUNCTION__,
          "mnl_socket_open", errno);
    }

    if (mnl_socket_bind(nl, 0, MNL_SOCKET_AUTOPID) < 0) {
        throw ndConntrackSystemException(__PRETTY_FUNCTION__,
          "mnl_socket_bind", errno);
    }
...
void ndConntrackFlow::Update(struct nf_conntrack *ct) {
    ...
    if (nfct_attr_is_set(ct, ATTR_ORIG_PORT_SRC))
        orig_port[ndCT_DIR_SRC] = nfct_get_attr_u16(ct,
          ATTR_ORIG_PORT_SRC);
    if (nfct_attr_is_set(ct, ATTR_ORIG_PORT_DST))
        orig_port[ndCT_DIR_DST] = nfct_get_attr_u16(ct,
          ATTR_ORIG_PORT_DST);
    ...
    if (nfct_attr_is_set(ct, ATTR_REPL_PORT_SRC))
        repl_port[ndCT_DIR_SRC] = nfct_get_attr_u16(ct,
          ATTR_REPL_PORT_SRC);
    if (nfct_attr_is_set(ct, ATTR_REPL_PORT_DST))
        repl_port[ndCT_DIR_DST] = nfct_get_attr_u16(ct,
          ATTR_REPL_PORT_DST);
