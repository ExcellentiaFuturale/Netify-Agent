...
static int nd_ct_event_callback(
  ...
  if (now > nd_ct_last_flow_purge_ttl) {
    ...
    nd_ct_last_flow_purge_ttl =
        now + _ND_CT_FLOW_TTL + (_ND_CT_FLOW_TTL / 10);
  }

  return (thread->ShouldTerminate()) ? NFCT_CB_STOP
                                     : NFCT_CB_CONTINUE;
}

static int nd_ct_netlink_callback(
    const struct nlmsghdr *nlh, void *data) {
  struct nf_conntrack *ct = nfct_new();

  if (ct == NULL)
    throw ndConntrackSystemException(__PRETTY_FUNCTION__,
                                     "nfct_new", errno);
...
ndConntrackThread::ndConntrackThread(int16_t cpu)
    : ndThread("nd-conntrack", (long)cpu),
      ...
      cb_registered(-1) {
  cth =
      nfct_open(NFNL_SUBSYS_CTNETLINK, NFCT_ALL_CT_GROUPS);
  if (cth == NULL) {
  ...
  setsockopt(ctfd, SOL_NETLINK,
             NETLINK_BROADCAST_SEND_ERROR, &on,
             sizeof(int));
  ...
  if ((cb_registered = nfct_callback_register(
           cth, NFCT_T_ALL, nd_ct_event_callback,
           (void *)this)) < 0) {
...
ndConntrackThread::~ndConntrackThread() {
  ...
  if (cth != NULL) {
    if (cb_registered != -1) nfct_callback_unregister(cth);
    nfct_close(cth);
  }

  for (nd_ct_flow_map::const_iterator i =
           ct_flow_map.begin();
       i != ct_flow_map.end(); i++)
    delete i->second;
...
void ndConntrackThread::DumpConntrackTable(void) {
  ...
  if (errno == EINVAL)
    nd_printf(
        "Is the nf_conntrack_netlink kernel module "
        "loaded?\n");

  while (rc > 0) {
    rc = mnl_cb_run(buffer, rc, seq, portid,
...
void ndConntrackThread::PrintFlow(ndConntrackFlow *flow,
  ...
  sprintf(buffer, "l3_proto: %hu, l4_proto: %hhu",
          flow->l3_proto, flow->l4_proto);
  os << buffer;

  switch (flow->orig_addr[ndCT_DIR_SRC].ss_family) {
    case AF_INET:
      sa_src = (struct sockaddr_in *)&flow
                   ->orig_addr[ndCT_DIR_SRC];
      sa_dst = (struct sockaddr_in *)&flow
                   ->orig_addr[ndCT_DIR_DST];
      if (reorder) {
        addr_cmp =
            memcmp(&sa_src->sin_addr, &sa_dst->sin_addr,
                   sizeof(in_addr));
        if (addr_cmp < 0) {
      ...
      break;
    case AF_INET6:
      sa6_src = (struct sockaddr_in6 *)&flow
                    ->orig_addr[ndCT_DIR_SRC];
      sa6_dst = (struct sockaddr_in6 *)&flow
                    ->orig_addr[ndCT_DIR_DST];
      if (reorder) {
        addr_cmp =
            memcmp(&sa6_src->sin6_addr, &sa6_dst->sin6_addr,
  ...
  if (reorder) {
    if (addr_cmp < 0) {
      os << ", lower_port: "
         << ntohs(flow->orig_port[ndCT_DIR_SRC]);
      os << ", upper_port: "
         << ntohs(flow->orig_port[ndCT_DIR_DST]);
    } else {
      os << ", lower_port: "
         << ntohs(flow->orig_port[ndCT_DIR_DST]);
      os << ", upper_port: "
  ...
  if (!withreply || !flow->repl_addr_valid[ndCT_DIR_SRC] ||
      !flow->repl_addr_valid[ndCT_DIR_DST]) {
    text = os.str();
    return;
  }
  ...
  switch (flow->repl_addr[ndCT_DIR_SRC].ss_family) {
    case AF_INET:
      sa_src = (struct sockaddr_in *)&flow
                   ->repl_addr[ndCT_DIR_SRC];
      sa_dst = (struct sockaddr_in *)&flow
                   ->repl_addr[ndCT_DIR_DST];
      inet_ntop(AF_INET, &sa_src->sin_addr.s_addr, ip,
                INET_ADDRSTRLEN);
      os << ", repl_src_ip: " << ip;
      inet_ntop(AF_INET, &sa_dst->sin_addr.s_addr, ip,
  ...
  os << ", repl_src_port: "
     << ntohs(flow->repl_port[ndCT_DIR_SRC]);
  os << ", repl_dst_port: "
     << ntohs(flow->repl_port[ndCT_DIR_DST]);

  text = os.str();
}

void ndConntrackThread::PrintFlow(nd_flow_ptr &flow,
                                  string &text) {
  ostringstream os;
  char buffer[1024];
  sa_family_t family;
  ...
  sprintf(buffer, "l3_proto: %hu, l4_proto: %hhu", family,
          flow->ip_protocol);
...
void ndConntrackThread::UpdateFlow(nd_flow_ptr &flow) {
  ...
  sa_family_t family;
  struct sockaddr_in *sa_orig_src = NULL,
                     *sa_orig_dst = NULL;
  struct sockaddr_in *sa_repl_src = NULL,
                     *sa_repl_dst = NULL;
  struct sockaddr_in6 *sa6_orig_src = NULL,
                      *sa6_orig_dst = NULL;
  struct sockaddr_in6 *sa6_repl_src = NULL,
                      *sa6_repl_dst = NULL;
  nd_ct_flow_map::iterator flow_iter;
  ...
  sha1_write(&ctx, (const char *)&family,
             sizeof(sa_family_t));
...
void ndConntrackFlow::Update(struct nf_conntrack *ct) {
  ...
  if (!nfct_attr_is_set(ct, ATTR_ORIG_L3PROTO))
    throw ndConntrackFlowException(
        "ATTR_ORIG_L3PROTO not set");

  sa_family_t af = l3_proto =
...
void ndConntrackFlow::Hash(void) {
  ...
  sha1_write(&ctx, (const char *)&l3_proto,
             sizeof(sa_family_t));
  sha1_write(&ctx, (const char *)&l4_proto,
             sizeof(uint8_t));
  ...
  switch (orig_addr[ndCT_DIR_SRC].ss_family) {
    case AF_INET:
      ...
      if (addr_cmp < 0) {
        sha1_write(&ctx, (const char *)&sa_src->sin_addr,
                   sizeof(struct in_addr));
        sha1_write(&ctx, (const char *)&sa_dst->sin_addr,
                   sizeof(struct in_addr));
      } else {
        sha1_write(&ctx, (const char *)&sa_dst->sin_addr,
                   sizeof(struct in_addr));
  ...
  if (addr_cmp < 0) {
    sha1_write(&ctx, (const char *)&repl_port[ndCT_DIR_SRC],
               sizeof(uint16_t));
    sha1_write(&ctx, (const char *)&repl_port[ndCT_DIR_DST],
               sizeof(uint16_t));
  } else {
    sha1_write(&ctx, (const char *)&repl_port[ndCT_DIR_DST],
               sizeof(uint16_t));
