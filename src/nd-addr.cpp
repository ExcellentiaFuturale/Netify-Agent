...
bool ndAddr::Create(ndAddr &a, const string &addr) {
  ...
  if ((p = addr.find_first_of("/")) != string::npos) {
    try {
      a.prefix =
          (uint8_t)stoul(addr.substr(p + 1), nullptr, 10);
    } catch (...) {
  ...
  if (inet_pton(AF_INET6, _addr.c_str(),
                &a.addr.in6.sin6_addr) == 1) {
...
bool ndAddr::Create(ndAddr &a, const uint8_t *hw_addr,
                    size_t length) {
  switch (length) {
    case ETH_ALEN:
  ...
  switch (ss_addr->ss_family) {
    case AF_INET:
      ...
      if (prefix)
        ...
        a.prefix = _ND_ADDR_BITSv4;

      memcpy(&a.addr.in, ss_addr,
    ...
    case AF_INET6:
      ...
      if (prefix)
        ...
        a.prefix = _ND_ADDR_BITSv6;

      memcpy(&a.addr.in6, ss_addr,
...
bool ndAddr::Create(ndAddr &a,
  ...
  memcpy(&a.addr.in6.sin6_addr, in6_addr,
...
const uint8_t *ndAddr::GetAddress(void) const {
  ...
  if (IsIPv6()) return (const uint8_t *)&addr.in6.sin6_addr;
...
uint16_t ndAddr::GetPort(bool byte_swap) const {
  if (!IsValid()) return 0;
  if (IsIPv4())
    return ((byte_swap) ? ntohs(addr.in.sin_port)
                        : addr.in.sin_port);
  if (IsIPv6())
    return ((byte_swap) ? ntohs(addr.in6.sin6_port)
                        : addr.in6.sin6_port);
...
bool ndAddrType::RemoveAddress(const ndAddr &addr,
                               const string &ifname) {
  if (!addr.IsValid()) {
    nd_printf("Invalid address: %s\n",
              addr.GetString().c_str());
...
void ndAddrType::Classify(ndAddr::Type &type,
                          const ndAddr &addr) {
  if (addr.IsValid())
    type = ndAddr::atOTHER;
  else {
  ...
  if (addr.IsEthernet()) {
...
#if defined(__linux__)
      if ((i & addr.addr.ll.sll_addr[0]) != i) continue;
#elif defined(__FreeBSD__)
      if ((i &
    ...
    if (ndRadixNetworkEntry<_ND_ADDR_BITSv4>::CreateQuery(
      ...
      if ((it = ipv4_reserved.longest_match(entry)) !=
          ipv4_reserved.end()) {
        type = it->second;
        if (type == ndAddr::atLOCALNET && !addr.IsNetwork())
          type = ndAddr::atLOCAL;
        return;
      }
    }
...
size_t ndAddrType::GetInterfaceAddresses(
...
size_t ndInterface::UpdateAddrs(
