...
ndPlugin::ndPlugin(Type type, const string &tag,
                   const Params &params)
...
ndPluginSink::ndPluginSink(const string &tag,
    ...
    : ndPlugin(ndPlugin::TYPE_SINK, tag, params),
      plq_size(0),
      plq_size_max(_ND_PLQ_DEFAULT_MAX_SIZE) {
  int rc;
  ...
  if ((rc = pthread_cond_init(&plq_cond, &cond_attr)) != 0)
    throw ndPluginException("pthread_cond_init",
                            strerror(rc));

  pthread_condattr_destroy(&cond_attr);

  if ((rc = pthread_mutex_init(&plq_cond_mutex, NULL)) != 0)
    throw ndPluginException("pthread_mutex_init",
                            strerror(rc));
...
ndPluginSinkPayload *ndPluginSinkPayload::Create(
  ...
  if (!(flags & ndPlugin::DF_GZ_DEFLATE))
    p = new ndPluginSinkPayload(length, data, channels,
                                flags);
  else {
    vector<uint8_t> buffer;
    nd_gz_deflate(length, data, buffer);
    p = new ndPluginSinkPayload(buffer.size(), &buffer[0],
                                channels, flags);
  }
...
size_t ndPluginSink::WaitOnPayloadQueue(unsigned timeout) {
  ...
  if (timeout > 0 && entries == 0) {
    ...
    if ((rc = pthread_mutex_lock(&plq_cond_mutex)) != 0) {
      throw ndPluginException("pthread_mutex_lock",
                              strerror(rc));
    }

    struct timespec ts_cond;
    if (clock_gettime(CLOCK_MONOTONIC, &ts_cond) != 0) {
      throw ndPluginException("clock_gettime",
                              strerror(errno));
    }
    ...
    if ((rc = pthread_cond_timedwait(
        ...
        rc != ETIMEDOUT) {
      throw ndPluginException("pthread_cond_timedwait",
                              strerror(rc));
    }

    if ((rc = pthread_mutex_unlock(&plq_cond_mutex)) != 0) {
      throw ndPluginException("pthread_mutex_unlock",
                              strerror(rc));
    }
...
void ndPluginProcessor::DispatchSinkPayload(
  ...
  throw ndPluginException("sink target not found",
                          target.c_str());
}
...
void ndPluginProcessor::DispatchSinkPayload(
  ...
  if ((flags & ndPlugin::DF_FORMAT_MSGPACK)) {
    ...
    if ((flags & ndPlugin::DF_ADD_HEADER)) {
      ...
      DispatchSinkPayload(target, channels, header, flags);
    }
    ...
    if ((flags & ndPlugin::DF_ADD_HEADER)) {
      ...
      DispatchSinkPayload(target, channels, header.size(),
...
ndPluginLoader::ndPluginLoader(
    const string &tag, const string &so_name,
    const ndPlugin::Params &params)
...
void ndPluginManager::Load(ndPlugin::Type type,
                           bool create) {
  ...
  for (auto &t : ndPlugin::types) {
    ...
    for (auto &i : *plugins) {
      ...
      loader = new ndPluginLoader(i.first, i.second.first,
                                  i.second.second);
      ...
      switch (t.first) {
        ...
        case ndPlugin::TYPE_SINK:
          ...
          break;
        default:
          throw ndPluginException(i.first, "invalid type");
          break;
      }
  ...
  for (auto &t : ndPlugin::types) {
    ...
    switch (t.first) {
      ...
      case ndPlugin::TYPE_SINK:
        ...
        break;
      default:
        throw ndPluginException(t.second, "invalid type");
        break;
    }
    ...
    if (pl == mp->end()) {
      throw ndPluginException(t.second, "plugin not found");
    }
...
void ndPluginManager::BroadcastSinkPayload(
  ...
  for (; p != prev(sinks.cend()); p++) {
    ndPluginSinkPayload *sp =
        ndPluginSinkPayload::Create(payload);
