...
ndPacketRing::ndPacketRing(const string &ifname,
  ...
  : ifname(ifname), sd(-1), buffer(nullptr), tp_hdr_len(0),
    ...
    {
        nd_dprintf(
          "%s: setsockopt(PACKET_VERSION): TPACKET_V3: "
          "%s\n",
          ifname.c_str(), strerror(errno));
        throw ndCaptureThreadException(
    ...
    for (unsigned i = 0; i < pmreq_types.size(); i++) {
        ...
        {
            nd_dprintf(
              "%s: setsockopt(PACKET_ADD_MEMBERSHIP:%hu): "
              "%s\n",
...
#warning "PACKET_FANOUT not supported."
    ...
    if (tp_reserved != sizeof(struct vlan_tag)) {
        nd_dprintf(
          "%s: PACKET_RESERVE: unexpected size: %lu != "
          "%u\n",
...
ndCaptureTPv3::ndCaptureTPv3(int16_t cpu, nd_iface_ptr &iface,
  const nd_detection_threads &threads_dpi,
  ndDNSHintCache *dhc, uint8_t private_addr)
  : ndCaptureThread(ndCT_TPV3, cpu, iface, threads_dpi, dhc,
      private_addr),
    ring(nullptr) {
    dl_type = DLT_EN10MB;
