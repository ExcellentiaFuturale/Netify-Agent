...
class ndPacketRingBlock {
 public:
  ...
  inline void SetStatus(
      uint32_t status = TP_STATUS_KERNEL) {
...
size_t ndPacketRingBlock::ProcessPackets(
    ndPacketRing *ring, vector<ndPacket *> &pkt_queue) {
...
ndPacketRing::ndPacketRing(const string &ifname,
  ...
  if (setsockopt(sd, SOL_PACKET, PACKET_VERSION,
                 (const void *)&so_uintval,
                 sizeof(so_uintval)) < 0) {
    nd_dprintf(
        "%s: setsockopt(PACKET_VERSION): TPACKET_V3: %s\n",
        ifname.c_str(), strerror(errno));
    throw ndCaptureThreadException(
        "error selecting TPACKET_V3");
  }
  ...
  if (bind(sd, (const struct sockaddr *)&sa_ll_bind,
    ...
    nd_dprintf("%s: bind: %s\n", ifname.c_str(),
               strerror(errno));
    throw ndCaptureThreadException(
        "unable to bind socket to interface");
  }
  ...
  for (unsigned i = 0; i < pmreq_types.size(); i++) {
    ...
    if (setsockopt(sd, SOL_PACKET, PACKET_ADD_MEMBERSHIP,
      ...
      nd_dprintf(
          "%s: setsockopt(PACKET_ADD_MEMBERSHIP:%hu): %s\n",
          ifname.c_str(), pmreq.mr_type, strerror(errno));
      throw ndCaptureThreadException(
          "error while adding membership(s)");
    }
  }
#ifdef PACKET_FANOUT
...
#else
    nd_dprintf(
        "%s: PACKET_FANOUT_FLAG_ROLLOVER not supported.\n",
        ifname.c_str());
#endif
    so_uintval <<= 16;
...
#warning "PACKET_FANOUT not supported."
  ...
  if (setsockopt(sd, SOL_PACKET, PACKET_RESERVE,
    ...
    nd_dprintf("%s: setsockopt(PACKET_RESERVE): %s\n",
               ifname.c_str(), strerror(errno));
    throw ndCaptureThreadException(
        "error reserving VLAN TAG space");
  }
  ...
  if (getsockopt(sd, SOL_PACKET, PACKET_RESERVE,
    ...
    nd_dprintf("%s: getsockopt(PACKET_RESERVE): %s\n",
               ifname.c_str(), strerror(errno));
    throw ndCaptureThreadException(
        "error getting reserved VLAN TAG size");
  }

  tp_reserved = (size_t)so_uintval;
  if (tp_reserved != sizeof(struct vlan_tag)) {
    nd_dprintf(
        "%s: PACKET_RESERVE: unexpected size: %lu != %u\n",
        ifname.c_str(), tp_reserved,
        sizeof(struct vlan_tag));
    throw ndCaptureThreadException(
        "unexpected reserved VLAN TAG size");
  }
  ...
  if (setsockopt(sd, SOL_PACKET, PACKET_RX_RING,
                 (const void *)&tp_req,
                 sizeof(struct tpacket_req3)) < 0) {
    nd_dprintf("%s: setsockopt(PACKET_RX_RING): %s\n",
               ifname.c_str(), strerror(errno));
    throw ndCaptureThreadException(
        "error requesting RX ring");
  }

  buffer =
      mmap(0, tp_req.tp_block_size * tp_req.tp_block_nr,
...
bool ndPacketRing::ApplyFilter(const uint8_t *pkt,
  ...
  return (filter.bf_insns &&
          bpf_filter(filter.bf_insns, pkt, length,
                     snaplen) == 0);
}
...
ndPacketRingBlock *ndPacketRing::Next(void) {
  ...
  if ((*it_block)->hdr.bdh->hdr.bh1.block_status &
      TP_STATUS_USER) {
    block = (*it_block);
...
ndPacket *ndPacketRing::CopyPacket(
    const void *entry, ndPacket::status_flags &status) {
  const struct tpacket3_hdr *hdr =
      (const struct tpacket3_hdr *)entry;
  ...
  if ((hdr->hv1.tp_vlan_tci ||
    ...
    memmove((void *)data, data + sizeof(struct vlan_tag),
            _ND_VLAN_OFFSET);
  ...
  if (pkt)
    status |= ndPacket::STATUS_OK;
  else
    status = ndPacket::STATUS_ENOMEM;
...
ndCaptureTPv3::ndCaptureTPv3(
    ...
    : ndCaptureThread(ndCT_TPV3, cpu, iface, threads_dpi,
                      dhc, private_addr),
      ring(nullptr) {
  dl_type = DLT_EN10MB;
    ...
    if (pkt_queue.size()) {
      ...
      try {
        for (auto &pkt : pkt_queue) {
          if (ProcessPacket(pkt) != nullptr) delete pkt;
        }
      } catch (...) {
        Unlock();
        capture_state = STATE_OFFLINE;
