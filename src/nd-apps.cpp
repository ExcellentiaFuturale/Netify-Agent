...
typedef radix_tree<ndRadixNetworkEntry<32>, nd_app_id_t>
    nd_rn4_app;
typedef radix_tree<ndRadixNetworkEntry<128>, nd_app_id_t>
    nd_rn6_app;
...
ndApplications::~ndApplications() { Reset(true); }
...
bool ndApplications::Load(const string &filename) {
  ...
  while (getline(ifs, line)) {
    ...
    if (type == "app" || type == "dom" || type == "net") {
      ...
      line = line.substr(0, p);
      signed aid = (signed)strtol(line.c_str(), NULL, 0);
  ...
  if (stats.ac > 0) {
    nd_dprintf(
        "Loaded %u apps, %u domains, %u networks, %u "
        "soft-dissectors, %u transforms.\n",
...
bool ndApplications::LoadLegacy(const string &filename) {
  ...
  while (getline(ifs, line)) {
    ...
    if (apps.find(app_id) == apps.end()) {
      if (AddApp(app_id, app_tag) != nullptr)
        ac++;
      else
        return false;
    }
  ...
  if (ac > 0) {
    nd_dprintf(
        "Loaded [legacy] %u apps, %u domains, %u networks, "
        "%u transforms.\n",
        ac, dc, nc, xc);
  }
...
bool ndApplications::Save(const string &filename) {
  ...
  for (auto &it : apps)
    ofs << "app:" << it.first << ":" << it.second->tag
        << endl;
  for (auto &it : domains)
  ...
  for (auto &it : domain_xforms)
    ofs << "xfm:" << it.first << ":" << it.second.second
        << endl;

  nd_dprintf(
      "Exported %u apps, %u domains, %u networks, %u "
      "transforms.\n",
      apps.size(), domains.size(), nc,
      domain_xforms.size());
...
nd_app_id_t ndApplications::Find(const ndAddr &addr) {
  ...
  if (addr.IsIPv6()) {
    ...
    if (ndRadixNetworkEntry<128>::CreateQuery(entry,
                                              addr)) {
      lock_guard<mutex> ul(lock);
...
bool ndApplications::Lookup(const string &tag,
                            ndApplication &app) {
  lock_guard<mutex> ul(lock);
...
bool ndApplications::Lookup(nd_app_id_t id,
                            ndApplication &app) {
  lock_guard<mutex> ul(lock);
...
ndApplication *ndApplications::AddApp(nd_app_id_t id,
                                      const string &tag) {
...
bool ndApplications::AddDomain(nd_app_id_t id,
                               const string &domain) {
...
bool ndApplications::AddDomainTransform(
  ...
  try {
    ...
    nd_printf(
        "WARNING: Error compiling domain transform regex: "
        "%s: %s [%d]\n",
...
bool ndApplications::AddNetwork(nd_app_id_t id,
...
bool ndApplications::SoftDissectorMatch(
    nd_flow_ptr const &flow, ndFlowParser *parser,
