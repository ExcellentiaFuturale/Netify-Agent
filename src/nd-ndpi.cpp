...
void ndpi_global_init(void) {
  ...
  if (ndpi_get_api_version() != NDPI_API_VERSION)
    throw ndThreadException(
        "nDPI library version mis-match");
  ...
  if (it != ndGC.protocols.end()) {
    ...
    } else if (strcasecmp(it->second.c_str(), "exclude") ==
               0) {
  ...
  for (auto it : ndGC.protocols) {
    ...
    if (id == ND_PROTO_UNKNOWN) {
      id = nd_ndpi_proto_find(
          (unsigned)strtoul(it.first.c_str(), NULL, 0));
...
struct ndpi_detection_module_struct *nd_ndpi_init(void) {
  ...
  if (ndGC_DEBUG) {
    ...
    if (!ndGC_QUIET) ndpi->ndpi_log_level = NDPI_LOG_DEBUG;
    if (ndGC_DEBUG_NDPI)
  ...
  ndpi_set_detection_preferences(
      ndpi, ndpi_pref_enable_tls_block_dissection, 1);
  ndpi_set_detection_preferences(
      ndpi, ndpi_pref_direction_detect_disable, 0);
