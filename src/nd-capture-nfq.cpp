...
static int ndCaptureNFQueue_Callback(
  ...
  ndCaptureNFQueue *nfq =
      static_cast<ndCaptureNFQueue *>(user);
  ...
  if (attr[NFQA_TIMESTAMP] != nullptr) {
    pkt_ts =
        static_cast<struct nfqnl_msg_packet_timestamp *>(
  ...
  pkt = new ndPacket(ndPacket::STATUS_OK, pkt_caplen,
  ...
  struct nlmsghdr *nlh_verdict =
      nfq_nlmsg_put((char *)buffer, NFQNL_MSG_VERDICT,
                    ntohs(nfg->res_id));
...
ndCaptureNFQueue::ndCaptureNFQueue(
    ...
    unsigned instance_id, ndDNSHintCache *dhc,
    uint8_t private_addr)
    : ndCaptureThread(ndCT_NFQ, cpu, iface, threads_dpi,
                      dhc, private_addr),
      nl(nullptr),
      port_id(0),
  ...
  nlh = nfq_nlmsg_put((char *)buffer, NFQNL_MSG_CONFIG,
                      queue_id);
...
void *ndCaptureNFQueue::Entry(void) {
  ...
  while (!ShouldTerminate()) {
    ...
    rc = mnl_socket_recvfrom(nl, (char *)buffer,
                             buffer_size);
...
void ndCaptureNFQueue::GetCaptureStats(
    ndPacketStats &stats) {
