...
#if _ND_DEBUG_FHC
      nd_dprintf(
          "Purging flow hash cache entries, size: %lu\n",
          lookup.size());
#endif
      for (size_t n = 0;
           n < cache_size / ndGC.fhc_purge_divisor; n++) {
        pair<string, string> j = index.back();

        nd_fhc_map::iterator k = lookup.find(j.first);
        if (k == lookup.end()) {
          nd_dprintf(
              "WARNING: flow hash cache index not found in "
              "map\n");
        } else
          lookup.erase(k);
...
void ndFlowHashCache::Load(void) {
  ...
  switch (ndGC.fhc_save) {
    case ndFHC_PERSISTENT:
      filename =
          ndGC.path_state_persistent + ND_FLOW_HC_FILE_NAME;
      break;
    case ndFHC_VOLATILE:
      filename =
          ndGC.path_state_volatile + ND_FLOW_HC_FILE_NAME;
      break;
    default:
      return;
  }
