...
void *nd_mem_alloc(size_t size) { return malloc(size); }

void nd_mem_free(void *ptr) { free(ptr); }

void nd_output_lock(void) { nd_printf_mutex.lock(); }

void nd_output_unlock(void) { nd_printf_mutex.unlock(); }
...
void nd_ndpi_debug_printf(uint32_t protocol, void *ndpi,
                          ...
                          const char *format, ...) {
  if (ndGC_DEBUG &&
      (ndGC_DEBUG_NDPI || level == NDPI_LOG_ERROR)) {
    lock_guard<mutex> lock(nd_printf_mutex);
...
int ndDebugLogBufferUnlocked::sync() {
  if (!os.str().empty()) {
    if (ndGC_DEBUG) fprintf(stderr, "%s", os.str().c_str());
    os.str("");
  }
...
int nd_sha1_file(const string &filename, uint8_t *digest) {
  ...
  close(fd);
  memcpy(digest, sha1_result(&ctx, _digest),
         SHA1_DIGEST_LENGTH);
...
void nd_sha1_to_string(const vector<uint8_t> &digest_bin,
                       string &digest_str) {
  nd_sha1_to_string((const uint8_t *)&digest_bin[0],
                    digest_str);
}
...
sa_family_t nd_string_to_ip(const string &src,
  ...
  if (inet_pton(AF_INET, src.c_str(), &ipv4->sin_addr) == 1)
    family = AF_INET;
  else if (inet_pton(AF_INET6, src.c_str(),
                     &ipv6->sin6_addr) == 1)
    family = AF_INET6;
  ...
  switch (ip.ss_family) {
    case AF_INET:
      return nd_ip_to_string(
          AF_INET, (const void *)&ipv4->sin_addr.s_addr,
          dst);
    case AF_INET6:
      return nd_ip_to_string(
          AF_INET6, (const void *)&ipv6->sin6_addr.s6_addr,
          dst);
    default:
      return false;
  }
...
bool nd_is_ipaddr(const char *ip) {
  ...
  return (inet_pton(AF_INET6, ip, &addr6) == 1) ? true
                                                : false;
}
...
void nd_private_ipaddr(uint8_t index,
  ...
  if (addr.ss_family == AF_INET) {
    ...
    rc =
        inet_pton(AF_INET, os.str().c_str(), &sa->sin_addr);
  } else if (addr.ss_family == AF_INET6) {
    os << ND_PRIVATE_IPV6 << hex << (int)index;
    struct sockaddr_in6 *sa =
        reinterpret_cast<struct sockaddr_in6 *>(&addr);
    rc = inet_pton(AF_INET6, os.str().c_str(),
                   &sa->sin6_addr);
  }
...
bool nd_save_uuid(const string &uuid, const string &path,
  ...
  if (fwrite((const void *)uuid.c_str(), 1, length, fh) !=
      length) {
    fclose(fh);
...
void nd_seed_rng(void) {
  ...
  if (fh == nullptr)
    ...
    if (fread((void *)&seed, sizeof(unsigned int), 1, fh) !=
        1)
...
void nd_generate_uuid(string &uuid) {
  ...
  while (input != 0) {
    result.push_front(
        toupper(clist[input % strlen(clist)]));
    input /= strlen(clist);
  }
...
pid_t nd_is_running(pid_t pid, const string &exe_base) {
  ...
  if (sysctlnametomib("kern.proc.pathname", mib, &length) <
      0) {
  ...
  if (sysctl(mib, 4, pathname, &length, nullptr, 0) == -1) {
    ...
    if (exe_base.size() < length) length = exe_base.size();

    if (strncmp(pathname_base, exe_base.c_str(), length) ==
        0) {
      rc = pid;
    } else
      rc = 0;
...
int nd_functions_exec(const string &func, const string &arg,
  ...
  if (ph != nullptr) {
    ...
    do {
      if ((bytes = fread(buffer, 1, sizeof(buffer), ph)) >
          0)
  ...
  if (stat(path.c_str(), &sb) == -1) {
    if (errno == ENOENT) {
      if (mkdir(path.c_str(), 0750) != 0)
        throw ndSystemException(__PRETTY_FUNCTION__, path,
                                errno);
    } else
      throw ndSystemException(__PRETTY_FUNCTION__, path,
                              errno);
...
void nd_set_hostname(char *dst, const char *src,
  ...
  strncpy(dst, buffer.c_str(),
          min(length, buffer.length()));
}
...
void nd_gz_deflate(size_t length, const uint8_t *data,
  ...
  if (rc != Z_STREAM_END) {
    throw ndSystemException(__PRETTY_FUNCTION__, "deflate",
                            EINVAL);
  }
#if 0
    nd_dprintf(
...
int nd_glob(const string &pattern,
